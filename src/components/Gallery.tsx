  ...
  return <>
      <section id="gallery" className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div ref={titleReveal.ref} className={`text-center mb-16 scroll-reveal ${titleReveal.isVisible ? 'revealed' : ''}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            const ImageCard = () => {
              ...
              return <div ref={cardReveal.ref} key={index} className={`group relative aspect-square overflow-hidden cursor-pointer bg-card border-2 border-border hover:border-primary transition-all duration-300 scroll-scale ${cardReveal.isVisible ? 'revealed' : ''}`} style={{
      ...
      {/* Lightbox Modal */}
      {selectedImage && <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={() => setSelectedImage(null)}>
          <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 w-12 h-12 rounded-full glass-strong flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-[var(--transition-smooth)] z-10" aria-label="Fechar">
            <X className="w-6 h-6" />
          </button>
